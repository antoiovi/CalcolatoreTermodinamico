 ...
 * @param init
 * @param end
 * @param step passo di distanza dei punti. S � un INT allora viene chiaata l'altra funzione che 
 ...
 */
	 ...
	 * @param init
	 * @param end
	 * @param number NUMERO DI ELEMENTI I CUI DIVIDERE LA SCALA. Se � double allora viene chiaat 
		...
		try {
			...
			for (int x = 0; x < z; x++) {
				...
				for (int y = 1; y < 9; y++) {
					...
					//	la fine di una diecina � l'inizio della successiva
					k = 10 * x + y - x;
					double dd = d + y * d;
					a[k] = Math.log10(dd);
				}
