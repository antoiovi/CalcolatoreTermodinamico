 ...
 *         implementare men� labels UTILIZZO: 
...
public class APanelDiagram extends JPanel implements ItemListener, ActionListener,APDiagram {
	 ...
	 * di adapt_scale; � quella usata nei calcoli, e di default vale x_scale
	 */
	private double x_scale_adapted;
	/**
	 * Y_scale_adapted: scala calcolata in base alle opzioni impostate di
	 * adapt_scale; � quella usata nei calcoli per evetuamente adattare allo
...
// Color.RED
// Color.BLUE
// new Color(30,144,255)  // blu più moderno
 ...
 */
	...
	************************************************************* */
	...
	public void paintComponent(Graphics g) {
		...
		/**
		 * Creo l'array delle coordinate, VERR� INIZIALIZZATO IN BASE AALLA
		 * SCAALA
		 */
	...
	//			+ margin_bottom);
		...
		/**
		 * Nuova larghezza DISEGNO: Larghezza dell'asse x disegnato: sar� la
...
private void paintTransitionBand(Graphics2D g2d) {
    ...
    // Se l'asse non è inizializzato esco
    ...
    // Se la banda è fuori dal range asse X, non disegno
    if (reMax < x_axis[0] || reMin > x_axis[x_axis.length - 1])
        return;

    // Conversione coordinate reali → coordinate schermo
...
g2d.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING,
	...
	/***************
	 * Disegno le etichette asse X , conoscendo gi� i rect delle etichette ;
	...
	/***************
	 * Disegno le etichette asse Y , conoscendo gi� i rect delle etichette ;
	 * verifihe se vengono sovrapposte ed in base alle opzioni decido come
	 * stamparle a video ( per ora scrivo solo le etichette che non si
	 * sovrappongono).
	 * 
	 ...
	 * @param popup
	 *            - il menu, che in base alla posizione verr� visualizzzato
	 */
	private  void addPopup(Component component, final JPopupMenu popup) {
		component.addMouseListener(new MouseAdapter() {
			...
			/**
			 * se il clic �a sinistra dell'asse Y o sotto l'asse X visualizzo il
			...
			private void showMenu(MouseEvent e) {
				if(!menu_abilitated)
					return;
				/**
				 * Per verificare se il clic � fuori dall'area diagram e
				 * nell'are label utilizzo le coordinate del pannello Poich� le
 ...
 * che � separato per permettere operazioni sui dati senza modificarli
 * @param x_axis
 */
 ...
 * che � separato per permettere operazioni(per la visualizzazione) sui dati senza modificarli
 * 
 * @param y_axis
 */
	 ...
	 */
		public void setFormatNumberAxis(String formatNumberAxis) {
			...
			try {
				String.format(formatNumberAxis, 0.15);// Verifica se nuovo formato �
														// corretto
	...
	public void setFormatNumberAxisX(String formatNumberAxisX) {
		...
		try {
			String.format(formatNumberAxisX, 0.15);// Verifica se nuovo formato �
										// corretto
	...
	public void setFormatNumberAxiY(String formatNumberAxiY) {
		...
		try {
			String.format(formatNumberAxiY, 0.15);// Verifica se nuovo formato �
										// corretto
